...
- 本机协议基线由 `protocol/rpc.rs` 的版本常量控制；新增严格事件枚举后使用 v3，拒绝不能解析这些事件的 v2 进程混用，更新时 UI/agent/daemon 必须一起重启。`task_activity.rs` 定义任务运行态与活动 DTO；`daemon.task.activity {taskId, beforeId?, afterId?, limit?}` 查询持久历史，两种游标互斥、页内 ID 升序，返回保留范围、更多页及截断信息。实时 `TaskActivityAdded` 仅在提交后广播，`TaskRuntimeChanged` 同时更新快照。
